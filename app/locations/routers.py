...
from app.organizations.dependencies import OrganizationContext, get_organization_context
...
def create_location(
    ...
    new_location = Location(
        organization_id=org_context.organization_id, **location.model_dump()
    )
...
def debug_location_access(
    ...
    return {
        ...
        "same_organization": (
            location.organization_id == org_context.organization_id
            if location
            else False
        ),
        "can_access": location is not None
        and location.organization_id == org_context.organization_id,
    }
...
def get_locations(
    ...
    if org_context.can_access_all_organizations():
        ...
        locations = location_repo.get_by_organization(
            org_context.organization_id, skip, limit
        )
